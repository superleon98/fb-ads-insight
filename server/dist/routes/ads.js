...
const adlibrary_1 = require("../services/adlibrary");
 ...
 */
router.post('/analyze', rateLimit_1.rateLimit, (0, cache_1.cacheMiddleware)('ads'), async (req, res) => {
    try {
        ...
        const validChannels = channels.filter(c => ['meta', 'google', 'tiktok'].includes(c));
        ...
        // 并行抓取各渠道广告
        const rawByChannel = await (0, adlibrary_1.fetchAdsByChannels)(brandName, validChannels, daysBack);
        // 转换为各渠道报告
        const channelReports = validChannels.map(channel => {
            const rawAds = rawByChannel[channel] || [];
            return (0, analyzer_1.transformToChannelReport)(brandName, channel, rawAds);
        });
        // 汇总数据
