...
const kimi_1 = require("../services/kimi");
 ...
 * 需要登录
 */
router.post('/analyze', auth_1.requireAuth, rateLimit_1.rateLimit, async (req, res) => {
    try {
        ...
        const result = await (0, kimi_1.analyzeWithKimi)(brandName, ads, channel);
        res.json(result);
    }
    catch (error) {
 ...
 */
router.post('/analyze-summary', auth_1.requireAuth, rateLimit_1.rateLimit, async (req, res) => {
    try {
        ...
        res.status(500).json({ error: '汇总分析失败', message: error.message });
    }
});
