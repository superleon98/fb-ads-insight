...
function detectFormat(adsType: string): AdCreative['format'] {
  switch (adsType) {
    case '2': return 'video';
...
function detectIndustry(pageName: string): string {
  const n = pageName.toLowerCase();
  if (n.includes('tech') || n.includes('app')) return '科技';
  if (n.includes('fashion') || n.includes('style')) return '时尚';
  if (n.includes('beauty') || n.includes('skin')) return '美妆';
  if (n.includes('fitness') || n.includes('gym')) return '健康';
  if (n.includes('food') || n.includes('meal')) return '餐饮';
  if (n.includes('edu') || n.includes('course')) return '教育';
  return '电商';
}

function analyzeKeywords(ads: AdLibraryAd[]): { word: string; count: number }[] {
...
function analyzeCtas(ads: AdLibraryAd[]): { cta: string; count: number }[] {
  const counts: Record<string, number> = {};
  ads.forEach(a => {
...
function analyzePlatforms(ads: AdLibraryAd[]): { platform: string; percentage: number }[] {
...
function analyzeRegions(ads: AdLibraryAd[]): { country: string; percentage: number }[] {
  const counts: Record<string, number> = {}; let total = 0;
  ads.forEach(a => {
    (a.geo || []).forEach(g => {
...
function generateTimeline(ads: AdLibraryAd[]): TimelineEvent[] {
  const events: TimelineEvent[] = [];
  const now = new Date();
  for (let i = 90; i >= 0; i -= 7) {
    const date = new Date(now); date.setDate(date.getDate() - i);
    const ds = date.toISOString().split('T')[0];
    const ws = new Date(date); ws.setDate(ws.getDate() - 7);
...
function makeInsights(brand: string, metrics: BrandMetrics, ca: CreativeAnalysis, ads: AdLibraryAd[]): string[] {
...
function makeRecommendations(ca: CreativeAnalysis, metrics: BrandMetrics): string[] {
  const recs: string[] = [];
  ...
  if (!hasCarousel) recs.push('增加轮播广告用于多产品展示，轮播广告转化率通常高于单图 20-30%');
  if (!hasVideo) recs.push('引入视频素材进行测试，视频广告的停留时长和记忆度显著优于静态图片');
  if (metrics.topRegions.length < 5) recs.push('考虑向新兴高增速市场拓展地域覆盖');
  if (ca.ctaDistribution.length < 4) recs.push('丰富 CTA 组合，针对不同漏斗阶段测试差异化引导语');
  if (ca.avgCopyLength < 80) recs.push('尝试加长教育型文案，在讲故事的同时植入产品利益点');
  recs.push('建立竞对监控机制，每周追踪上新素材和文案角度');
  return recs.slice(0, 6);
}

export function transformToChannelReport(brandName: string, channel: AdPlatform, rawAds: AdLibraryAd[]): ChannelReport {
  const now = new Date();
  const ads: AdData[] = rawAds.map((raw, i) => {
  ...
  const pattern: StrategyAnalysis['spendingPattern'] =
    active.length > 40 && avgImp > 50000 ? 'aggressive' :
  ...
  const creativeAnalysis: CreativeAnalysis = {
