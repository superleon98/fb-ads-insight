...
import type { AdData, AIAnalysisResult, AdPlatform } from '../types';
 ...
 */
function buildPrompt(brandName: string, ads: AdData[], channel: AdPlatform): string {
  ...
  const formatDist = {} as Record<string, number>;
  const ctaDist = {} as Record<string, number>;
  const regionDist = {} as Record<string, number>;
 ...
 * 解析 Kimi 返回文本为结构化结果
 */
function parseAIResponse(text: string, brandName: string, channel: AdPlatform): AIAnalysisResult {
  const channelNames: Record<string, string> = {
    meta: 'Meta',
    google: 'Google',
    tiktok: 'TikTok',
